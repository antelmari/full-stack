...
--
-- Table structure for table `users_plots`
--

CREATE TABLE `users_plots` (
...
--
-- Dumping data for table `users_plots`
-- 

INSERT INTO `users_plots`(`user_id`, `plot_id`)
SELECT `users`.`user_id`, `users`.`plot_id`
FROM users
    JOIN plots ON (`users`.`plot_id` = `plots`.`number`);
